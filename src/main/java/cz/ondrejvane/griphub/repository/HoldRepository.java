...
public interface HoldRepository extends JpaRepository<Hold, Long> {
    ...
    @Query(value = "select hold from Hold hold left join fetch hold.wall", countQuery = "select count(hold) from Hold hold")
    Page<Hold> findAllWithToOneRelationships(Pageable pageable);

    @Query("select hold from Hold hold left join fetch hold.wall")
    List<Hold> findAllWithToOneRelationships();

    @Query("select hold from Hold hold left join fetch hold.wall where hold.id =:id")
